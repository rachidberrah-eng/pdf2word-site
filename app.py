...
# مجلد مؤقت لكل عملية معالجة (يعمل على Windows و Linux)
...
def process():
    ...
    try:
        ...
        subprocess.run(
            [pandoc_cmd, md_path, f"--reference-doc={ref_path}", "-o", output_path],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        if "pandoc" in error_msg.lower() or "not found" in error_msg.lower():
            return jsonify({
                "error": "لم يتم العثور على Pandoc. يرجى تثبيته من https://pandoc.org/installing.html"
            }), 500
        return jsonify({"error": f"فشل توليد ملف Word: {error_msg}"}), 500
    except FileNotFoundError as e:
        if "pandoc" in str(e).lower():
            return jsonify({
                "error": "Pandoc غير موجود. يرجى تثبيته من https://pandoc.org/installing.html"
            }), 500
